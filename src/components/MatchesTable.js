...
class MatchesTable extends Component {
  ...
  matchRankChangesByResult = () => {
    ...
    for (const result of results) {
      ...
      const matchesWithResult = this.props.matches.filter(
  ...
  showThrowerLeaverColumn = () => {
    const matches = this.props.matches.filter(
      match =>
        match.allyThrower ||
        match.allyLeaver ||
        match.enemyThrower ||
  ...
  showDayTimeColumn = () => {
    return (
      this.props.matches.filter(match => match.dayOfWeek && match.timeOfDay)
        .length > 0
    );
  };
  ...
  showHeroesColumn = () => {
    return (
      this.props.matches.filter(match => match.heroList.length > 0).length > 0
    );
  };
  ...
  showGroupColumn = () => {
    return (
      this.props.matches.filter(
        match => match.groupList.length > 0 || match.groupSize > 1
      ).length > 0
    );
  };
  ...
  getLongestWinStreak = () => {
    const winStreaks = this.props.matches
      .filter(match => typeof match.winStreak === "number")
  ...
  getLongestLossStreak = () => {
    const lossStreaks = this.props.matches
      .filter(match => typeof match.lossStreak === "number")
